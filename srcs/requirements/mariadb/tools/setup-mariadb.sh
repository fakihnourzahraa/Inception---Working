...
until mysqladmin ping -u root --silent 2>/dev/null || mysqladmin ping -u root -p"$(cat /run/secrets/db_root_password)" --silent 2>/dev/null; do
    sleep 1
done
echo "MariaDB is ready"

if ! mysql -u root -p"$(cat /run/secrets/db_root_password)" -e "USE $DB_NAME;" 2>/dev/null; then
